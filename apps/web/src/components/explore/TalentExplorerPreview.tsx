      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-4 lg:pt-8">
          <p className="font-sans text-base sm:text-lg text-[#6E6678] leading-relaxed">
            An interactive hiring surface where evidence matters more than
            claims.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-[#D9CEDF] rounded-[34px] p-6 sm:p-8 shadow-[0_24px_54px_rgba(30,91,255,0.06)] space-y-6">
        <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-6 pb-6 border-b border-[#D9CEDF]/60">
          ...
          <div className="flex flex-wrap gap-2 items-center">
            ...
            <span className="bg-white border border-[#D9CEDF] text-[#6E6678] rounded-full px-3 py-1 text-xs font-mono">
              Remote
            </span>
            <span className="bg-white border border-[#D9CEDF] text-[#6E6678] rounded-full px-3 py-1 text-xs font-mono">
              Available now
            </span>
            <span className="font-mono text-xs text-[#6E6678] ml-4 hidden xl:inline">
              1,280 matching professionals
            </span>
          </div>
        </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-sm font-semibold text-[#17131F] truncate leading-tight">
                      {c.name}
                    </h4>
                    <p className="text-xs text-[#6E6678] mt-0.5 truncate">
                      {c.role}
                    </p>
                  </div>
                  {isSelected && (
                    <span className="font-mono text-[9px] text-[#1E5BFF] uppercase tracking-wider font-semibold">
                      Active
                    </span>
                  )}
                </div>
              ...
              <div className="md:col-span-5 bg-[#F7F9FF] border border-[#D9CEDF]/60 rounded-2xl p-5 space-y-3">
                ...
                {selectedCandidate.evidence.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-xs text-[#6E6678] font-medium">
                      {item}
                    </span>
                  </div>
                ))}
