...
export const EnglishLevelEnum = z.enum(
  ["BASIC", "CONVERSATIONAL", "PROFESSIONAL", "FLUENT", "NATIVE"] as const,
  {
...
export const createExperienceSchema = z.object({
  ...
  endDate: z
    .string()
    .datetime({ message: "Invalid endDate ISO string" })
    .optional()
    .nullable(),
...
export const createEducationSchema = createEducationSchemaRaw.refine(
...
export const updateEducationSchema = createEducationSchemaRaw
  .partial()
  .refine(
    (data) =>
      !data.endYear || !data.startYear || data.endYear >= data.startYear,
    {
      message: "End year must be greater than or equal to start year",
      path: ["endYear"],
    },
