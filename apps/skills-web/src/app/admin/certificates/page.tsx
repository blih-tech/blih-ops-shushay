  ...
  return (
    <main className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <AdminTable<AdminCertificate>
        ...
        emptySubtext={searchQuery ? "Try a different search term." : "No certificates issued yet."}
        columns={[
          {
            key: "recipient",
            header: "Recipient",
              ...
              <div className="flex items-center gap-3 min-w-0">
                <div className="w-8 h-8 rounded-xl bg-[#EEF3FF] text-[#1E5BFF] flex items-center justify-center text-sm font-bold shrink-0 overflow-hidden">
                  {c.user.talentProfile?.photoUrl ? (
                    ...
                    <img src={c.user.talentProfile.photoUrl} alt="" className="w-full h-full object-cover" />
                  ) : (
                    recipientName(c).charAt(0).toUpperCase()
                  )}
                </div>
                <div className="min-w-0">
                  <p className="font-medium text-[#17131F] truncate">{recipientName(c)}</p>
          ...
          },
          {
            ...
            render: (c) => (
              <span className="font-mono text-xs text-[#6E6678] truncate max-w-[220px] block" title={c.certificateNumber}>
                {c.certificateNumber}
              </span>
            ),
          },
